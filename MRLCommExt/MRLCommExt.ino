...
*/
/* Modificaciones
** - Utiliza ServoExt que permite indicar en el constructor el valor por defecto del servo para que al encenderlo no se
**   posicione siempre en el medio
** - Se crea SERVO_ATTACH_WITH_DEFAULT para attachar el servo con un valor por defecto
*/
#include <ServoExt.h>
...
#define SERVO_ATTACH_WITH_DEFAULT         	100
	...
	if (getCommand())
	{
		...
		case SERVO_ATTACH:{
			...
			break;
                ...
                case SERVO_ATTACH_WITH_DEFAULT:{
			servo_type& s = servos[ioCmd[1]];
			s.index = ioCmd[1];
			if (s.servo == NULL){
				s.servo = new Servo(ioCmd[3]);
                                s.targetPos = ioCmd[3];
                                s.currentPos = ioCmd[3];
			}
			s.servo->attach(ioCmd[2]);
			s.step = 1;
			s.eventsEnabled = false;
